...
const Layout = ({user,onLogout}) => {
  ...
  const fetchTasks =useCallback(async ()=>{
    ...
    try {
      ...
      const {data} =await axios.get('http://127.0.0.1:8000/api/tasks/gp',{
...
const StatCard =({title,value,icon})=>{
  return(
    <div className="p-2 sm:p-3 rounded-xl bg-white shadow-sm border
    border-purple-100 transition-all duration-300 hover:border-purple-100 group">
      <div className="flex items-center gap-2">
        <div className="p-1.5 rounded-lg bg-gradient-to-br
        from-fuchsia-500/10 to-purple-500/10 
        group-hover:from-fuchsia-500/10 group-hover:to-purple-500/20">
          {icon}
        </div>
...
if(error)return(
  <div className="min-h-screen bg-gray-50 flex items-center justify-center">
    <div className="bg-red-50 text-red-600 p-4 rounded-xl border border-red-500 max-w-md">
      <p className="font-medium mb-2">
        Error loading tasks</p>
        <span className="text-sm ">{error}</span>
        <button onClick={fetchTasks} className='mt-4 py-2 px-4 bg-red-100 text-red-700
        rounded-lg font-medium hover:bg-red-200 transition-colors'>
         Try Again
        </button>
  ...
  return (
    <div className='min-h-screen bg-gray-50'>
      ...
      <div className='ml-0 xl:ml-64 lg:ml-64 md:ml-16
      pt-16 p-3 sm:p-4 md:p-4 transition-all duration-300'>
        ...
        <div className='xl:col-span-2 space-y-3 sm:space-y-4'>
          ...
          <div className="xl:col-span-1 space-y-4 sm:space-y-6">
            <div className="bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-purple-100">
              ...
              sm:mb-4 text-gray-800 flex items-center gap-2">
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6">
                ...
                <div className="space-y-2 sm:space-y-3">
                  <div className="flex items-center justify-between text-gray-700">
                    <span className="text-xs sm:text-sm font-medium flex items-center gap-5">
                  ...
                  <div className="bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-purple-100">
                    ...
                    <div className="space-y-2 sm:space-y-3">
                      {tasks.slice(0,3).map((task)=>(
                        ...
                        transition-color duration-200 border 
                        border-transparent hover:border-purple-100'>
                          ...
                          <span className={`px-2 py-1 text-xs rounded-full shrink-0 ml-2 ${task.completed ? 'bg-green-100 text-green-700'
                      ...
                      {tasks.length === 0 && (
                        <div className="text-center py-4 sm:py-6 px-2">
                          <div className="w-12 h-12 sm:h-16 mx-auto sm:mb-4 rounded-full
