...
const TaskModal = ({isOpen,onClose,taskToEdit,onSave,onLogout}) => {
  ...
  useEffect(()=>{
    if(!isOpen) return
    if (taskToEdit) {
  const normalized = taskToEdit.completed === 'Yes' || taskData.completed === true ? 'Yes' : 'No';
  setTaskData({
    ...DEFAULT_TASK,
  ...
  const handleSubmit = useCallback(async(e)=>{
    ...
    try {
      ...
      setError(err.message || 'un expected error occurred')
  ...
  return (
    ...
    z-50 flex items-center justify-center p-4">
     ...
     <form onSubmit={handleSubmit} className='space-y-4'>
      {error &&  <div className="text-sm text-red-600 bg-red-50 
       border border-red-100 p-3 rounded-lg">{error}</div>}
       <div>
        ...
        <div>
          <label className="flex items-center gap-1
           text-sm font-medium text-gray-700 mb-1">
             <AlignCenter className='w-4 h-4 text-purple-500'/>Description 
        </label> 
        ...
        <div>
           ...
           text-sm font-medium text-gray-700 mb-1">
            ...
            <input type='date' name='dueDate' required min={today}
            value={taskData.dueDate} onChange={handleChange}
        ...
        <button type='submit' disabled={loading}
         ...
         text-white font-medium py-2.5 px-4 rounded-lg flex
